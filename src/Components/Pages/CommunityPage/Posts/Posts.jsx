...
export default function Posts({ allPosts, refetch }) {
  ...
  async function likePost(postId) {
    try {
      await axios.post(
        `https://beige-wildcat-74200.zap.cloud/api/posts/${postId}/like`,
        {},
        {
          headers: {
  ...
  async function deletePost(postId) {
    try {
      await axios.delete(
        `https://beige-wildcat-74200.zap.cloud/api/posts/${postId}`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
          },
...
async function handleCommentSubmit(postId) {
  ...
  try {
    await axios.post(
      `https://beige-wildcat-74200.zap.cloud/api/posts/${postId}/comment`,
      { comment: content },
      {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("token")}`,
            ...
            return (
              ...
              >
                ...
                <div className="flex items-center border shadow-sm rounded-lg bg-white bg-opacity-70 border-gray-200 justify-around mt-4">
                  ...
                  <Link to={'/post/'+post.id} className={'flex items-center my-2 gap-2 text-gray-600 text-lg'}>
                  <FaArrowRightLong />
